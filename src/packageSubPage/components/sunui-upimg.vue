...
<script>
	...
	export default {
		...
		props: {
			...
			url: { type: String, default: 'https://a3.dns06.net.cn/app/index.php?i=2&c=entry&a=wxapp&do=Upload_qiniu_b&m=jzwx_a' },
			upload_auto: { type: Boolean, default: true },
			signature: { type: Object, default: () => {} },
			upload_before_list: { type: Array, default: () => [] } // 上传数组
		},
		created() {
			_self = this;
		},
	...
	const upload = options => {
		var url = options.url,
			...
			fail = options.fail
		var key = path && path.split('/');
		key = key && key[key.length - 1];
		extra.key = key;
